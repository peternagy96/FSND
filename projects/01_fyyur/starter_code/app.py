...
def venues():
  ...
  unique_cities = [{'city': x.city, 'state': x.state} for x in venues]
  unique_cities = [dict(t) for t in {tuple(d.items()) for d in unique_cities}]
  for c in unique_cities:
    for v in venues:
      if v.city == c['city'] and v.state == c['state']:
        if c.get('venues', None) is None:
          c['venues'] = []
        c['venues'].append({'id': v.id, 'name': v.name})
...
def show_artist(artist_id):
  ...
  #query_venues = Venue.query.join(Show).filter(Show.artist_id == artist_id).all()
...
def shows():
  ...
  #   "start_time": "2035-04-15T20:00:00.000Z"
  # }]
  #query_shows = Artist.query.join(Venue).filter((Venue.shows.artist == Artist.id) & (Artist.shows.venue == Venue.id)).all()
